...
export class ServiceOptionService {
  ...
  async create(
    createServiceOptionDto: CreateServiceOptionDto,
    ...
    if (!gym) {
      throw new NotFoundException(
        `Gym with ID ${createServiceOptionDto.gymId} not found`,
      );
    }
    ...
    if (ownerOrAdmin?.role !== 'ADMIN') {
      if (gym?.gymOwnerId !== currentUserId) {
        throw new ForbiddenException(
          `Cannot create a service for a gym you do not own`,
        );
      }
    }
  ...
  async update(
    id: number,
    updateServiceOptionDto: UpdateServiceOptionDto,
    currentUserId: number,
  ) {
    ...
    if (ownerOrAdmin?.role !== 'ADMIN') {
      if (gym?.gymOwnerId !== currentUserId) {
        throw new ForbiddenException(
          `Cannot update service option you do not own`,
        );
      }
    }
  ...
  async remove(id: number, currentUserId: number) {
    ...
    if (ownerOrAdmin?.role !== 'ADMIN') {
      if (serviceOption?.gym.gymOwnerId !== currentUserId) {
        throw new ForbiddenException(
          `Cannot delete a service option you do not own`,
        );
      }
    }
