...
export class ServiceOptionController {
  ...
  @ApiResponse({
    status: 409,
    description: 'Option name already exists for this gym ',
  })
  ...
  @ApiResponse({
    status: 409,
    description: 'Updated name conflicts with another option in the same gym.',
  })
  @ApiBearerAuth('JWT-auth')
  update(
  ...
  @ApiResponse({
    status: 204,
    description: 'Service option successfully deleted.',
  })
  @ApiResponse({ status: 404, description: 'Service option not found.' })
  @ApiBearerAuth('JWT-auth')
  @HttpCode(204)
