...
export class RatingService {
  ...
  async create(createRatingDto: CreateRatingDto, userId: number) {
    ...
    if(!check){
      throw new BadRequestException("Cannot rate a product you have not purchased")
    }

    if(!validRatings.includes(createRatingDto.rating)){
      throw new BadRequestException("Rating value must be on of these values (1,2,3,4,5)")
    }
