...
export class CartService {
  ...
  async create(createCartItemDto: CreateCartItemDto, userId: number) {
    ...
    if (checkProduct.type !== ProductType.DIGITAL) {
      throw new BadRequestException(
        'Only Digital products can be added to cart',
      );
    }
  ...
  async remove(productId: number, userId: number) {
    const checkIfUserOwnsCart = await this.databaseService.cart.findUnique({
      where: {
        cartOwnerId: userId,
      },
      select: {
    ...
    const checkProductExistsInCart =
      await this.databaseService.cartItem.findUnique({
        where: {
          cartId_productId: {
            cartId: checkIfUserOwnsCart.id,
            productId: productId,
