...
export class ReviewsController {
  ...
  @ApiForbiddenResponse({
    description: 'Forbidden. User already reviewed this gym.',
  })
  createReview(@Body() dto: CreateReviewDto, @GetUser() user: User) {
  ...
  @ApiOperation({
    summary: 'Delete a review (Customer for own, Admin for any)',
  })
  @ApiParam({
    name: 'id',
  ...
  @ApiNotFoundResponse({ description: 'Review not found.' })
  @ApiForbiddenResponse({
    description: 'Forbidden. Can only respond to reviews for own gym.',
  })
  addResponse(
    @GetUser() user: User,
  ...
  @ApiParam({
    name: 'id',
    description: 'ID of the review to delete response from',
    type: Number,
  })
  @ApiOkResponse({ description: 'Response successfully deleted.' })
  @ApiNotFoundResponse({ description: 'Response not found.' })
  @ApiForbiddenResponse({
    description: 'Forbidden. Can only delete responses for own gym.',
  })
  deleteResponse(
    @GetUser() user: User,
  ...
  @ApiParam({
    name: 'id',
    description: 'ID of the review to update response for',
    type: Number,
  })
  ...
  @ApiNotFoundResponse({ description: 'Response not found.' })
  @ApiForbiddenResponse({
    description: 'Forbidden. Can only update responses for own gym.',
  })
  updateResponse(
    @GetUser() user: User,
    @Param('id', ParseIntPipe) responseId: number,
  ...
  @Get('gym/:gymId')
  @ApiOperation({
    summary: 'Get all reviews for a specific gym (Public/Guest access)',
  })
  ...
  getGymReviews(
    ...
    return this.reviewsService.getGymReviews(
      gymId,
      paginationDto.page,
      paginationDto.limit,
    );
