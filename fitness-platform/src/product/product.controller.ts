...
export class ProductController {
  ...
  async create(
    ...
    if (createProductDto.type === ProductType.DIGITAL) {
      if (!documentFile) {
        this.cleanupFiles(paths);
        throw new BadRequestException(
          'Digital products require a document file',
        );
      }
      ...
      if (documentFile) {
        this.cleanupFiles(paths);
        throw new BadRequestException(
          'Physical products cannot have a document attachment',
        );
      }
    }
  ...
  async getVendorSalesHistory(@Req() req: RequestWithUser) {
    return this.productService.getVendorRevenue(
      req.user.userId,
      req.user.isVendor,
    );
  }
  ...
  async update(
    ...
    const cleanupNewFiles = () => {
      ...
      if (documentFile?.path && fs.existsSync(documentFile.path))
    ...
    if (product.document) {
      ...
      if (!fs.existsSync(absolutePath)) {
        throw new NotFoundException(
          'The requested file is missing from the server.',
        );
      }
